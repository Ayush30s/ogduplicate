    ...
    if (currentuser) {
      ...
      allGyms.forEach((gym) => {
        ...
        if (gym.joinedby.length > 0) {
          hasJoined = gym.joinedby.some(
            (join) => join.user._id.toString() === userId.toString()
          );
        }
...
homeRoute.get("/joinedgyms", async (req, res) => {
  ...
  if (!mygyms) mygyms = await gymModel.findById(userId).populate("joinedgym");
...
homeRoute.get("/gym/:gymId", async (req, res) => {
  ...
  try {
    ...
    daysLeftToMonth =
      ...
      day +
      Number(daysInMonth) -
      (Number(daysInMonth) - Number(joinedDate));
...
homeRoute.get("/:gymId/joingym", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.redirect(
        `/home/gym/${gymId}?msg=You can't join, if you are an owner of any Gym, use another Id!`
      );
    }

    // Check if the user has already joined the gym
    const isUserJoined = gymData.joinedby.some(
      (joined) => `${joined.user._id}` === `${userId}`
    );
    ...
    return res.redirect(
      `/home/gym/${gymId}?success=Congratulations you have joined the gym successfully`
    );
  } catch (error) {
...
homeRoute.get("/mygyms", async (req, res) => {
  try {
    ...
    const Mygymdata = await gymModel
    ...
    if (!Mygymdata) {
      return res.redirect(
        "/home?msg=You do not have ownership of any gym. Please use a different ID to register a new gym."
      );
    }
...
homeRoute.get("/profile/:userId", async (req, res) => {
  ...
  if (usertype === "OWNER" && userData) {
    ...
    showjoinedgym = userData.joinedby.some(
      (joined) => joined.user.toString() === req.user._id.toString()
    );
  }
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    for (let j = 1; j <= days; j++) {
      ...
      userData.workout.forEach((day) => {
        ...
        if (
          Number(day.pushedAt.getMonth()) == Number(i) &&
          Number(day.pushedAt.getDate()) == Number(j)
        ) {
...
homeRoute.get("/:gymId/leavegym", async (req, res) => {
  ...
  try {
    ...
    await userModel.updateOne({ _id: userId }, { $pull: { joinedgym: gymId } });
  ...
  try {
    ...
    if (userJoinedOrNot) {
      ...
      if (limit > data.joinedby.length) {
        if (currentShift) {
          await ShiftModel.findByIdAndUpdate(currentShift._id, {
    ...
    if (msg != undefined) {
      ...
      return res.redirect(
        `/home/gym/${gymId}?success=${"Shift joined successfully"}`
      );
    }
  } catch (error) {
